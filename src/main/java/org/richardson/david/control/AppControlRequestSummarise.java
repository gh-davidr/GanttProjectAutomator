...
import org.richardson.david.entity.email.EmailMessageAlertSummarise;
import org.richardson.david.entity.email.EmailMessageInboxPlaceholderAll;
import org.richardson.david.entity.email.EmailMessageInboxPlaceholderSummarise;
...
public class AppControlRequestSummarise extends AppControlSummarise
{
	private static Logger LOGGER = LoggerFactory.getLogger(AppControlRequestSummarise.class);

	public AppControlRequestSummarise(Boolean reportOnlyBoolean) {
	...
	public int doSummarise(Boolean log)
	{
		return doSummarise(log, new EmailMessageInboxPlaceholderSummarise());	
	}
	
	public int doSummarise(Boolean log, EmailMessageInboxPlaceholderBase inboxMonitorPlaceholderEmail)
	{
		return doSummarise(new ReadEmailSession(
				EmailMessageInboxPlaceholderBase.addTag(
						new EmailMessageInboxPlaceholderSummarise().getInboxMonitorString(), 
						new EmailMessageInboxPlaceholderAll().getInboxMonitorString())),
				log,
				inboxMonitorPlaceholderEmail);	
	}
	
	private int doSummarise(ReadEmailSession accountEmails, Boolean log, EmailMessageInboxPlaceholderBase inboxMonitorPlaceholderEmail)
	{
		int summarizesRequested = 0;
		int alertsRaised = 0;
		...
		for (Message message : accountEmails.getFolderEmails().getOrderedMessages())
		{
			try {
				...
				{
					EmailMessageAlertSummarise alertEmail = new EmailMessageAlertSummarise(message);
					alertEmail.sendAlertEmail();
					alertsRaised++;

					LOGGER.info("  " + getRunMode() + "Resource NOT FOUND to request summarise: " + sender);
				}
				else 
				{					
					String subject = message.getSubject();

					if (Repository.getInstance().messageIsAPleaForSummarise(subject))
					{
						summarizesRequested++;
					}
				}
		...
		if (summarizesRequested + alertsRaised > 0 && inboxMonitorPlaceholderEmail != null)
		{
		...
		if (log)
		{
			if (summarizesRequested + alertsRaised > 0)
			{
			...
			LOGGER.info(getRunMode() + "Notify complete.  " +
					(reportOnlyBoolean ? "  Report Only Mode active, so no notification message sent"
							: (summarizesRequested == 1 ? "One notification message requested." : summarizesRequested + " notification messages requested.")));
		}

		return summarizesRequested + alertsRaised;
	}
